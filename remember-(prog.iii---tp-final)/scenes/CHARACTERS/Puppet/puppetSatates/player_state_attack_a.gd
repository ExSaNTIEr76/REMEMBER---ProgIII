...
class_name PlayerStateAttackA    extends PlayerStateBase
...
func start():
	...
	if current_state and current_state.name == player.promissio.states.Sleeping:
		...
		var symbol_scene = player.promissio.concrete_symbol_a
		if symbol_scene:
			...
			if PlayerManager.get_current_cp() >= cp_cost:
				...
				player.play_animation( player.animations.attackA + player.previous_direction, false )
				player.attack_a_timer.start()
			else:
				print( "❌ No hay CP suficiente para Attack A" )
				state_machine.change_to( player.states.Idle )
...
func on_process( _delta ):
	if player.attack_a_timer.time_left == 0:
		state_machine.change_to( player.states.Idle )
