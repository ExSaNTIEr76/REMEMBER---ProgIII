...
static func _deserialize_object(data, object = null):
	...
	# ❗ PATCH: Si es Resource, NO intentar asignar .name ni .transform
	if object is Resource:
		# Aplicar solo variables serializadas si existen
		if data.has("variables"):
			for k in data.variables:
				if object.has_method("set"):
					# Solo setear si la propiedad existe realmente
					if k in object.get_property_list().map(func(p): return p.name):
						object.set(k, _deserialize_variable(data.variables[k]))
		return object
