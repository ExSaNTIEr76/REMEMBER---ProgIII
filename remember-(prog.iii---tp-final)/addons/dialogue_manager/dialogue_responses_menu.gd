...
func _ready() -> void:
	visibility_changed.connect(func():
		if visible and get_menu_items().size() > 0:
			...
			if first_item.is_inside_tree() and first_item.focus_mode != Control.FOCUS_NONE:
...
func _configure_focus() -> void:
	...
	for i in items.size():
		...
		# Control del enfoque según si está bloqueado o no
		item.focus_mode = Control.FOCUS_ALL if not input_locked else Control.FOCUS_NONE
	...
	if not input_locked and items.size() > 0:
		items[0].grab_focus()
...
func _set_focus_enabled(enabled: bool) -> void:
	...
	for item in get_menu_items():
		item.focus_mode = Control.FOCUS_ALL if enabled else Control.FOCUS_NONE
...
func _on_response_gui_input(event: InputEvent, item: Control, response) -> void:
	if input_locked or "Disallowed" in item.name: return
